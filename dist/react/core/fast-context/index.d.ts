...
export declare const createFastContext: <T extends StoreDataType = StoreDataType>(debugName?: string, config?: CreateFastContextConfig) => {
    /**
     ...
     *
     */
    useContextValue: <SelectorOutput>(selector: Selector<T, SelectorOutput>, equalityCheck?: EqualityCheck) => [value: SelectorOutput, set: (value: (prev: T) => Partial<T>) => void];
...
export declare const createProvider: <T extends StoreDataType = StoreDataType>(StoreContext: Context<UseStoreDataReturnType<T>>, watch: boolean) => import("react").NamedExoticComponent<ProviderProps<T>> & {
    displayName: string | undefined;
};
export declare const createUseRefContext: <T extends StoreDataType = StoreDataType>(_Context: Context<UseStoreDataReturnType<T> | null>) => () => UseStoreDataReturnType<T>;
export declare const useFastContextStore: <IStore extends StoreDataType, SelectorOutput>(_Context: Context<UseStoreDataReturnType<IStore> | null>, selector: Selector<IStore, SelectorOutput>, equalityFn?: (value1: any, value2: any) => boolean) => [value: SelectorOutput, set: UseStoreDataReturnType<IStore>["set"]];
export declare const useStoreValue: <IStore extends StoreDataType, SelectorOutput>(_Context: Context<UseStoreDataReturnType<IStore> | null>, selector: Selector<IStore, SelectorOutput>, equalityFn?: (value1: any, value2: any) => boolean) => [value: SelectorOutput, set: (value: (prev: IStore) => Partial<IStore>) => void];
export {};
